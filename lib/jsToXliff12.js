...
function createTransUnitTag (key, resource, obj, options) {
  ...
  const srcVal = options.inlineAsString ? unflattenInline(resource.source) : resource.source
  if (srcVal != null) u.elements.push(makeElement('source', sourceAttributes, makeValue(srcVal, ElementTypes12)))
  if (resource.target != null) {
    ...
    u.elements.push(makeElement('target', targetAttributes, makeValue(options.inlineAsString ? unflattenInline(resource.target) : resource.target, ElementTypes12)))
  }
  if ('note' in resource) {
